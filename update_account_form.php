<?php 
    ...
    if($user_type == 'students'){
        ...
        $fac_users_query = "SELECT * FROM regular_users JOIN faculty ON regular_users.user_id=faculty.user_id JOIN departments ON departments.id=faculty.department_id".
        " WHERE regular_users.user_id=".$user_id.";";
        $result = mysql_query($fac_users_query);
        $fac_user_row = mysql_fetch_array($result); 
        
        $research_int_query = "SELECT * FROM research_interests WHERE faculty_id=".$user_id.";";
        $result = mysql_query($research_int_query);
        $research_int_row = mysql_fetch_array($result); 
        
        $fac_sections_query = "SELECT * FROM `faculty-sections` fs JOIN sections ON fs.section_id=sections.id JOIN courses ON sections.course_id=courses.id".
            " WHERE fs.faculty_id=".$user_id.";";
        $result = mysql_query($fac_sections_query);
        $fac_sections_row = mysql_fetch_array($result); 
    }
		...
		<form class="form-horizontal" action="update_account.php" method="post">
			...
			<div id="faculty_options">
			    <div class="control-group">
                    <label class="control-label" for="email">Email</label>
                    <div class="controls">
                        <input type="email" name="email" value="<?php echo $fac_user_row['email']; ?>">
                    </div>
                </div>
                <div class="control-group">
                    <label class="control-label" for="first_name">Name</label>
                    <div class="controls">
                        <input type="text" name="first_name" value="<?php echo $fac_user_row['first_name']; ?>">
                        <input type="text" name="last_name" value="<?php echo $fac_user_row['last_name']; ?>">
                    </div>
                </div>
                <div class="control-group">
                    <label class="control-label" for="dob">Date of Birth</label>
                    <div class="controls">
                        <input type="text" name="dob" value="<?php echo $fac_user_row['dob']; ?>">
                    </div>
                </div>
                <div class="control-group">
                    <label class="control-label" for="gender">Gender</label>
                    <div class="controls">
                        <input type="text" name="gender" value="<?php echo $fac_user_row['gender']; ?>">
                    </div>
                </div>
                <div class="control-group">
                    <label class="control-label" for="permanent_address">Permanent Address</label>
                    <div class="controls">
                        <input type="text" name="permanent_address_line1" value="<?php echo $fac_user_row['permanent_address']; ?>"><br/>
                    </div>
                </div>
                <div class="control-group">
                    <label class="control-label" for="current_address">Current Address</label>
                    <div class="controls">
                        <input type="text" name="current_address_line1" value="<?php echo $fac_user_row['current_address']; ?>"><br/>
                    </div>
                </div>
                <div class="control-group">
                    <label class="control-label" for="phone_number">Contact Number</label>
                    <div class="controls">
                        <input type="tel" name="phone_number" value="<?php echo $fac_user_row['phone_number']; ?>">
                    </div>
                </div>
				<div class="control-group">
					...
					<div class="controls">
						<select name="faculty_department">
							<?php $result = mysql_query("SELECT * FROM departments");
								while($row = mysql_fetch_array($result)){                                    
                                    if($fac_user_row['department_id'] == $row['id'] ){
                                        echo "<option value=".$row['id']." selected='selected'>".$row['name']."</option>";
                                    }
                                    else {
                                        echo "<option value=".$row['id'].">".$row['name']."</option>";
